...
function renderBullet(bulletConfig, videoEle, isCreate = false) {
  ...
  let spanLeft = getEleStyle(videoEle).width
  spanEle.style.left = spanLeft + 'px'
  // 实现弹幕的滚动
  let _timer = setInterval(() => {
    spanLeft -= bulletConfig.speed
    spanEle.style.left = spanLeft + 'px'
    if(spanLeft<0) {
...
document.querySelector("#sendBulletBtn").addEventListener("click", () => {
  ...
  const input = document.getElementById('bulletContent')
  bulletConfig.value = input.value
